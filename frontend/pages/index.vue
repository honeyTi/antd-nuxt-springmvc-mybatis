  ...
  <div class="home" :class="[model]">
    <div class="slide">
      ...
      <div ref="dots" class="swiper-dots">
        <span
          v-for="(dot,index) in slideData.length"
          :key="index"
          class="dot"
          :class="{active:currentSlideIndex===index}"
          @mouseenter="toSlide(index)"
        ></span>
      </div>
    </div>
    <div class="digital-panel">
      <div class="item">
        <div class="num">
          <em>{{countResult.totalNum}}</em>/份
        </div>
        <p class="name">设计、前端的各种灵感素材与知识库</p>
      </div>
      <div class="item">
        <div class="num">
          <em>{{countResult.todayNum}}</em>/张
        </div>
        <p class="name">今日新增</p>
      </div>
      <div class="item">
        <div class="num">
          <em>{{countResult.weekNum}}</em>/份
        </div>
        <p class="name">本周上传精品</p>
      </div>
      <div class="item">
        <div class="num">
          <em>{{mostUser.count}}</em>/份
        </div>
        <p class="name">
          本周上传最多
          <a :href="'/?postAuthor='+mostUser.userId">{{mostUser.username}}</a>
        </p>
      </div>
    </div>
    <div class="hot-style">
      ...
      <div class="tag-filter">
        ...
        <div class="shot-sizes">
          ...
          <ul class="large-small">
            <li
              title="Small with info"
              class="small-meta"
              :class="{active:model=='small'}"
              @click="changeMasonrySize('small')"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                ...
                class="icon"
              >
                <g stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="active">
                  <rect height="18" rx="2" width="17" x="5" y="5" />
                  <path d="m5 19h17v2c0 1.1046-.8954 2-2 2h-13c-1.10457 0-2-.8954-2-2z" />
                </g>
              </svg>
            </li>
            <li
              title="Large with info"
              class="large-meta"
              :class="{active:model=='big'}"
              @click="changeMasonrySize('big')"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill
              ...
              >
                <g stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="active">
                  <rect height="26" rx="2" width="26" x="1" y="1" />
                  <path d="m1 22h26v3c0 1.1046-.8954 2-2 2h-22c-1.10457 0-2-.8954-2-2z" />
                </g>
              </svg>
            </li>
          </ul>
    ...
    <div v-if="showLink" class="module-filter link-list">
      <link-label :link="links"></link-label>
    </div>
    <waterfall v-if="!showLink"></waterfall>
  </div>
</template>
<script>
import api from '~/assets/js/common/api'
// 引入link网址静态数据
import links from '~/assets/js/common/links'
import Swiper from 'swiper'
import 'swiper/dist/css/swiper.css'
import { message } from 'ant-design-vue'
import waterfall from '~/components/Waterfall'
import LinkLabel from '~/components/label/Label'
export default {
  ...
  data() {
    return {
      ...
      links: {
        img:
          'http://cdn.h5cool.cn/upload/admin/images/designernav/1516776526.jpg',
  ...
  mounted() {
    ...
    this.showLink = this.$route.query.termId === "9000"
  },
  methods: {
    // 切换瀑布流的模块大小
